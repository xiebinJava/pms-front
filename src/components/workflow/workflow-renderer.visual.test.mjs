...
test('story list workbench is a topic-management-only runtime component', () => {
  const registry = fs.readFileSync(new URL('./workflow-component-registry.ts', import.meta.url), 'utf8')
  assert.match(registry, /STORY_LIST: 'story-list'/)
  assert.match(registry, /label: '故事列表工作台'/)
  assert.match(registry, /processTypeCodes: \['topic-management'\]/)
})
