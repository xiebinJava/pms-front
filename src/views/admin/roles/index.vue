...
<template>
  <section class="admin-page pms-admin-page">
    ...
    <div class="pms-table-scroll pms-roles-table-scroll"><a-table class="pms-admin-table" :data-source="roles" :loading="loading" row-key="id">
      ...
      <a-table-column :title="$t('common.actions')" key="action"><template #default="{ record }"><a-button v-if="canRoleWrite" type="link" @click="edit(record)">{{ $t('common.edit') }}</a-button><a-button v-if="canRoleWrite && !record.builtin" type="link" danger @click="remove(record)">{{ $t('common.delete') }}</a-button><span v-if="!canRoleWrite" class="muted">{{ $t('admin.roles.noPermission') }}</span></template></a-table-column>
    </a-table></div>
