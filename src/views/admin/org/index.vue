...
const leaderSeedOptions = ref<PersonOption[]>([])
...
async function openEdit() {
  ...
  leaderSeedOptions.value = selected.value.leaderUserId && selected.value.leaderDisplayName
    ? [{ value: selected.value.leaderUserId, label: selected.value.leaderDisplayName }]
    : []
  editOpen.value = true
}
...
<template>
  <section class="admin-page pms-admin-page"><PmsPageHeader :title="$t('route.adminOrg')" :description="$t('admin.org.description')"><template #actions><a-button v-if="canWrite" class="pms-primary-button" @click="openCreate">{{ $t('admin.org.addUnit') }}</a-button></template></PmsPageHeader>
    ...
    <a-modal v-model:open="editOpen" :title="$t('admin.org.editTitle')" :ok-text="$t('common.save')" :cancel-text="$t('common.cancel')" :confirm-loading="editLoading" @ok="saveEdit"><a-form layout="vertical"><a-form-item :label="$t('admin.org.name')" required><a-input v-model:value="editForm.name" /></a-form-item><a-form-item :label="$t('admin.org.type')"><a-select v-model:value="editForm.typeCode" allow-clear style="width: 100%"><a-select-option value="BG">{{ $t('admin.org.typeBg') }}</a-select-option><a-select-option value="CENTER">{{ $t('admin.org.typeCenter') }}</a-select-option><a-select-option value="DEPARTMENT">{{ $t('admin.org.typeDepartment') }}</a-select-option><a-select-option value="TEAM">{{ $t('admin.org.typeTeam') }}</a-select-option><a-select-option value="PDT">{{ $t('admin.org.typePdt') }}</a-select-option></a-select></a-form-item><a-form-item :label="$t('admin.org.leader')"><PersonSelect v-model="editForm.leaderUserId" allow-clear :options="leaderSeedOptions" :placeholder="$t('admin.org.selectLeader')" /><a-checkbox v-model:checked="editForm.clearLeader">{{ $t('admin.org.clearLeader') }}</a-checkbox></a-form-item></a-form></a-modal>
