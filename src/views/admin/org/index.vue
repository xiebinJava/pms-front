...
<template>
  <section class="admin-page"><div class="page-heading"><div><h1>组织架构</h1><p>以主归属管理树形架构，兼职与 PDT 通过人员归属表达。</p></div><a-button v-if="canWrite" class="pms-primary-button" @click="openCreate">+ 新增组织单元</a-button></div>
    <div class="org-layout"><div class="org-panel"><OrgCanvas :units="tree" :selected="selected?.id" :readonly="!canWrite" @select="selected = $event" @move="moveTo" /></div><aside class="property-panel"><template v-if="selected"><div class="property-heading"><h2>{{ selected.name }}</h2><a-button v-if="canWrite" type="link" @click="openEdit">编辑</a-button></div><dl><dt>组织编码</dt><dd>{{ selected.code }}</dd><dt>组织类型</dt><dd>{{ selected.typeCode || '—' }}</dd><dt>负责人</dt><dd>{{ selected.leaderDisplayName || '—' }}</dd><dt>直属人员</dt><dd>{{ selected.memberCount ?? 0 }} 人</dd></dl><div v-if="canWrite" class="property-actions"><a-button @click="moveTo(selected.id, undefined)">移到顶层</a-button><a-button danger @click="remove">停用组织</a-button></div></template><a-empty v-else description="选择组织查看属性" /></aside></div>
    ...
    <a-modal v-model:open="editOpen" title="编辑组织属性" ok-text="保存" :confirm-loading="editLoading" @ok="saveEdit"><a-form layout="vertical"><a-form-item label="组织名称" required><a-input v-model:value="editForm.name" /></a-form-item><a-form-item label="组织类型"><a-select v-model:value="editForm.typeCode" allow-clear style="width: 100%"><a-select-option value="BG">业务群（BG）</a-select-option><a-select-option value="CENTER">中心</a-select-option><a-select-option value="DEPARTMENT">部门</a-select-option><a-select-option value="TEAM">团队</a-select-option><a-select-option value="PDT">PDT</a-select-option></a-select></a-form-item><a-form-item label="负责人"><a-select v-model:value="editForm.leaderUserId" allow-clear show-search option-filter-prop="label" placeholder="选择负责人" style="width: 100%"><a-select-option v-for="person in leaderOptions" :key="person.id" :value="person.id" :label="person.displayName">{{ person.displayName }}</a-select-option></a-select><a-checkbox v-model:checked="editForm.clearLeader">清空现有负责人</a-checkbox></a-form-item></a-form></a-modal>
  </section>
</template>
<style scoped>
