...
async function choose(event: Event) {
  ...
  if (!file) return
  loading.value = true
  try { preview.value = await previewImport(type.value, file); step.value = 1 } finally { loading.value = false }
}
...
<template><section class="admin-page"><div class="page-heading"><div><h1>批量导入</h1><p>支持 CSV / Excel；先预览校验，错误为零后一次性提交。</p></div><a-space><a-button @click="downloadTemplate">下载当前模板</a-button><a-radio-group v-model:value="type" button-style="solid"><a-radio-button value="organizations">组织</a-radio-button><a-radio-button value="users">员工</a-radio-button></a-radio-group></a-space></div><a-steps :current="step" size="small"><a-step title="上传" /><a-step title="预览校验" /><a-step title="完成" /></a-steps><div class="import-panel"><a-alert type="info" show-icon message="模板字段" :description="type === 'organizations' ? '组织编码、组织名称、组织类型编码、父组织编码、负责人英文名、排序' : '中文名、英文名、邮箱、手机号、主组织编码、岗位编码、角色编码、直属上级英文名'" /><label class="upload"><input type="file" accept=".csv,.xlsx" :disabled="loading" @change="choose" /><span>{{ loading ? '正在解析…' : '选择 CSV / Excel 文件' }}</span></label></div><template v-if="preview"><div class="preview-head"><strong>预览 {{ preview.rowCount }} 行</strong><a-tag :color="preview.errors.length ? 'red' : 'green'">{{ preview.errors.length ? `${preview.errors.length} 个错误` : '校验通过' }}</a-tag><a-space><a-button v-if="preview.errors.length" @click="downloadErrors">下载错误行</a-button><a-button class="pms-primary-button" :disabled="!!preview.errors.length" @click="commit">确认导入</a-button></a-space></div><a-table :data-source="preview.errors" row-key="row" size="small"><a-table-column title="行" data-index="row" /><a-table-column title="字段" data-index="field" /><a-table-column title="错误" data-index="message" /></a-table></template><a-result v-if="step === 2" status="success" title="导入完成" :sub-title="resultMessage" /></section></template>
