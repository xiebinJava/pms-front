...
<template>
  <section class="admin-page pms-admin-page">
    ...
    <a-modal v-model:open="inviteOpen" title="邀请员工" :confirm-loading="inviteLoading" ok-text="创建邀请" @ok="submitInvite">
      <a-form layout="vertical"><a-form-item label="邮箱" required><a-input v-model:value="form.email" placeholder="例如：name@example.com（登录账号）" /></a-form-item><a-form-item label="中文名"><a-input v-model:value="form.nameZh" placeholder="可选，例如：谢斌" /></a-form-item><a-form-item label="英文名"><a-input v-model:value="form.username" placeholder="可选，例如：Brad.Xie" /></a-form-item><a-form-item label="手机号"><a-input v-model:value="form.phone" /></a-form-item><a-form-item label="主归属" required><a-select v-model:value="form.orgUnitId" show-search option-filter-prop="label" placeholder="选择员工主归属" style="width: 100%"><a-select-option v-for="org in orgOptions" :key="org.id" :value="org.id" :label="org.name">{{ '　'.repeat(org.level) }}{{ org.name }}</a-select-option></a-select></a-form-item><a-form-item label="角色"><a-select v-model:value="form.roleCode" style="width: 100%"><a-select-option value="MEMBER">普通成员</a-select-option><a-select-option value="ORG_ADMIN">组织管理员</a-select-option></a-select></a-form-item></a-form>
