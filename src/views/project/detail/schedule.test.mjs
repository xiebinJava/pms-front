...
test('places project, nodes, iteration plans and dated tasks on the chart', () => {
  const model = buildScheduleModel({
    ...
    iterationPlans: [{ id: 9, name: 'Iteration 1', startDate: '2026-09-10', dueDate: '2026-09-15', status: 'PLANNED' }],
    tasks: [
  ...
  const iterationPlanLane = model.lanes.find((lane) => lane.kind === 'iteration-plan')
...
test('calendar keeps range bars on week rows and day chips on dates', () => {
  const events = buildCalendarEvents({
    ...
    iterationPlans: [{ id: 9, name: 'Iteration 1', startDate: '2026-09-01', dueDate: '2026-09-03', status: 'PLANNED' }],
  ...
  assert.deepEqual(wednesday?.events.map((event) => event.kind).sort(), ['iteration-plan', 'task'])
})
