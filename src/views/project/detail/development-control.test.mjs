...
test('mounts the development tree inside the develop node detail', () => {
  ...
  assert.match(workbench, /故事状态实时汇总/)
  assert.doesNotMatch(workbench, /所属迭代计划|迭代计划（可选）|历史/)
  assert.doesNotMatch(workbench, /getMilestones|所属里程碑|milestoneId|milestoneTitle/)
  assert.match(workbench, /story\.ownerName/)
  ...
  assert.match(workbench, /width="min\(640px, calc\(100vw - 32px\)\)"/)
  assert.doesNotMatch(workbench, /development-control__editor-story/)
  ...
  assert.doesNotMatch(workbench, /<a-range-picker/)
  assert.match(workbench, /getStoryStatusCounts\(topic\)/)
  assert.doesNotMatch(workbench, /@click="addStory"/)
  assert.doesNotMatch(workbench, /\+ 新增故事/)
  assert.doesNotMatch(workbench, /@click="removeStory\(/)
  assert.doesNotMatch(workbench, /@change="onStoryStatusChange\(story\)"/)
