...
async function submit() {
  ...
  loading.value = true
  try {
    await activate(String(route.query.token || ''), form.password)
    message.success(t('auth.activateSuccess'))
    router.replace('/login')
  } catch (error) {
    message.error((error as Error).message || t('auth.activateFailed'))
  } finally {
    loading.value = false
  }
}
...
<template>
  <div class="pms-auth-page activate-page">
    <div class="pms-auth-locale">
      <LocaleSwitch />
    </div>
    <div class="pms-auth-card activate-card">
      ...
      <a-form layout="vertical" @finish="submit">
        ...
        <a-button class="pms-primary-button" html-type="submit" block :loading="loading">{{ $t('auth.activateSubmit') }}</a-button>
      </a-form>
    </div>
  </div>
</template>
...
<style scoped>
.activate-card { width: min(420px, 100%); }
