...
5. 保留现有 `TASK_UPDATED` 审计记录，并额外写入 `TASK_DUE_DATE_CHANGED` 审计动作，before/after 只包含日期字段。
...
1. 后端先完成迁移、领域枚举、历史 Mapper、任务 DTO 和服务层写入逻辑。
2. 后端补齐任务详情、任务列表和工作台汇总测试，再更新 OpenAPI 文档。
3. 前端增加类型和国际化，再接入任务卡片和任务详情。
4. 接入甘特图、日历和工作台展示。
5. 用真实项目验证跨天、改期、完成、重新打开、版本冲突和只读项目场景。
6. 通过后端测试、前端测试、类型检查、构建和浏览器回归验证。
