...
- 参考项目：本地设计参考前端（未纳入本仓库）
- 核对文件：`src/styles/base.css`、`src/styles/layout.css`、`src/styles/components.css`、`src/layouts/AppShell.tsx`、`src/components/ui.tsx`
- 关键基线：62px 顶栏、236px 侧栏、1460px 内容宽度、`#f7f8fa` 页面底色、白色面板、8px 圆角、深蓝主色、1px 边框和双层轻阴影。

## 已完成映射

| 区域 | 迁移结果 |
| --- | --- |
| 令牌 | PMS 全局颜色、圆角、字体、阴影和控件高度对齐设计令牌；保留 Ant Design 作为行为层。 |
| 应用壳 | 顶栏、侧栏、内容区、移动端抽屉和遮罩改为统一主题几何关系。 |
...
本地 Vite 代理同时移除同源开发请求的 `Origin` 头，避免后端的 CORS 策略误判本地登录请求；不涉及业务接口或权限逻辑。

## 已知限制

当前视觉回归使用的是本地 MySQL 后端和管理员账号；未对业务数据、组织关系、角色权限或项目流程做写入操作。
