...
**Goal:** 将已确认的“需求澄清与范围基线”Demo 落到真实项目详情页，并让范围项、需求清单和基线确认状态可持久化。

> **后续变更（2026-09-05）**：经产品复核，项目目标和节点交付结果已在“项目立项与启动”节点维护，需求澄清节点不再重复录入。V20 迁移已物理删除 `project_node_baseline.objective`、`project_node_baseline.deliverable` 及其历史数据；当前节点只维护纳入/不纳入范围、需求清单和基线确认状态。
...
- 节点确认前允许保存草稿；确认要求至少一条纳入范围项、至少一条需求且所有需求为已确认。
...
**Files:**
- Create: `../pms-backend/src/main/resources/db/migration/V17__requirement_scope_baseline.sql`
- Create: `../pms-backend/src/main/java/com/brad/pms/entity/ProjectNodeBaselineDO.java`
- Create: `../pms-backend/src/main/java/com/brad/pms/entity/ProjectNodeScopeItemDO.java`
- Create: `../pms-backend/src/main/java/com/brad/pms/entity/ProjectNodeRequirementDO.java`
- Create: `../pms-backend/src/main/java/com/brad/pms/mapper/ProjectNodeBaselineMapper.java`
- Create: `../pms-backend/src/main/java/com/brad/pms/mapper/ProjectNodeScopeItemMapper.java`
- Create: `../pms-backend/src/main/java/com/brad/pms/mapper/ProjectNodeRequirementMapper.java`
- Create: `../pms-backend/src/main/java/com/brad/pms/dto/request/NodeScopeItemCmd.java`
- Create: `../pms-backend/src/main/java/com/brad/pms/dto/request/NodeRequirementCmd.java`
- Create: `../pms-backend/src/main/java/com/brad/pms/dto/request/NodeRequirementScopeUpdateCmd.java`
- Create: `../pms-backend/src/main/java/com/brad/pms/dto/response/NodeScopeItemDTO.java`
- Create: `../pms-backend/src/main/java/com/brad/pms/dto/response/NodeRequirementDTO.java`
- Create: `../pms-backend/src/main/java/com/brad/pms/dto/response/NodeRequirementScopeDTO.java`
- Create: `../pms-backend/src/main/java/com/brad/pms/controller/NodeRequirementScopeController.java`
- Create: `../pms-backend/src/main/java/com/brad/pms/service/NodeRequirementScopeService.java`
- Create: `../pms-backend/src/test/java/com/brad/pms/service/NodeRequirementScopeServiceTest.java`
- Modify: `../pms-backend/src/main/java/com/brad/pms/service/NodeService.java`
- Create: `../pms-backend/src/test/java/com/brad/pms/controller/NodeRequirementScopePermissionTest.java`

**Interfaces:**
- `GET /projects/{projectId}/nodes/{nodeId}/requirements-scope` returns `{ baselineStatus, confirmedAt, confirmedByName, scopeItems, requirements, canEdit }`.
- `PUT /projects/{projectId}/nodes/{nodeId}/requirements-scope` accepts `{ scopeItems: [{id?, direction, title, sort}], requirements: [{id?, code, name, description, type, priority, acceptanceCriteria, status, sort}] }` and returns the same DTO with generated ids.
...
**Files:**
- Create: `src/api/node-requirement-scope.ts`
- Create: `src/views/project/detail/components/RequirementScopeWorkbench.vue`
- Create: `src/views/project/detail/requirement-scope.ts`
- Create: `src/views/project/detail/requirement-scope.test.mjs`
...
- [ ] **Step 1: Write failing frontend tests for the pure rules and source integration.**
  ...
  ```js
  test('requires an in-scope item and confirmed requirements', () => {
    assert.equal(isRequirementBaselineComplete({ scopeItems: [], requirements: [] }), false)
    assert.equal(isRequirementBaselineComplete({ scopeItems: [{ direction: 'IN', title: '订单流程' }], requirements: [{ status: 1, name: '订单流转', acceptanceCriteria: '状态可追踪' }] }), true)
  })
  ```
...
- [ ] **Step 4: Build the workbench from the accepted Demo.**
  ...
  Recreate the accepted section order with code-native Ant Design Vue controls: scope in/out cards and requirements table. Do not add the objective/output already maintained by kickoff, the “业务问题与背景” or “参与角色”, or a redundant checklist footer. Keep copy i18n-backed in both Chinese and English. Use disabled controls for read-only/completed/terminated nodes, show save/confirm/reopen states, and surface server errors without losing the loaded form.
