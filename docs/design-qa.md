...
## 2026-09-22：专题与故事列表视觉重构

### 变更背景

专题管理原先只有基础查询表格，页头、筛选区、状态文案和空态没有完全复用项目列表的工作台规范。本次在不改变接口、路由、权限和数据关系的前提下，统一专题管理与故事管理的列表视觉和交互。

### 验收范围

- 页头增加“研发管理 / 工作项汇总”上下文，并提供专题/故事工作项切换。
- 列表面板增加列表标题、当前可见范围和准确记录总数；筛选控件统一紧凑高度与宽度。
- 专题、故事、项目/节点、负责人、状态、进度和行操作按同一信息层级展示；测试状态不再显示内部编码。
- 增加规范空态、键盘可达的真实按钮、焦点保留和窄屏下表格容器滚动。

### 阶段自检

- [x] 设计令牌沿用 `--pms-*` 语义变量，没有新增页面级颜色或阴影字面量。
- [x] 专题/故事切换、搜索回车、分页、详情跳转和状态展示保持原有数据行为。
- [x] 筛选控件容器明确使用 Flex 对齐和 8px 间距，搜索框、状态下拉和操作按钮不再黏连。
- [x] 已登录应用内浏览器桌面视口完成专题页和故事页截图验收，未出现空白页或框架错误遮罩。
- [x] `pnpm test`、`pnpm typecheck`、`pnpm build` 和 `git diff --check` 通过。

### 尚未验证

- 当前应用内浏览器工具没有提供独立的移动视口覆盖和控制台日志读取；窄屏 CSS 已补齐，但仍需在具备移动视口与控制台能力的环境中进行最终视觉回归。
