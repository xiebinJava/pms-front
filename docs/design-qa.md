...
- `index.css` 与 `pms-theme.css` 保持同一组全局 CSS 变量。
...
## 2026-09-15：Agent 配置入口与配置页重构

### 变更背景

Agent 配置需要成为「配置管理」中的独立页签，方便管理员从 PMS 壳层直接维护系统提示词、Skill 和插件。页面同时收敛为 PMS 的“页面头部—状态概览—配置工作区—固定操作区”层级，避免多个嵌套卡片造成信息噪声。

### 验收范围

- 配置管理新增「AI 助手」页签，使用 `admin:workflow:write` 权限，入口指向 `/ai-agents/project_assistant`。
- Agent 配置页提供当前 Agent、已发布版本和已启用能力概览；系统提示词、Skill、插件和运行策略按主次关系排列。
- 保存草稿与发布操作固定在工作区底部，页面滚动时仍保持可见；窄屏下工作区、扩展项和操作按钮自动换行。
- 页面颜色、间距、字号、边框、圆角和阴影统一复用 PMS 设计令牌；没有引入新的业务字段或权限边界。

### 阶段自检

- [x] 配置入口与路由权限一致，当前页使用 `aria-current="page"` 标记。
- [x] 设计令牌覆盖状态概览、扩展选择、策略说明和 sticky 操作区。
- [x] 1440px 桌面工作区与 640px 以下窄屏布局均有响应式规则。
- [x] 桌面使用已登录应用内浏览器、390px 使用等价接口快照完成截图验收：配置页无横向溢出，概览与扩展区按视口换行，操作区在底部保持可操作。
